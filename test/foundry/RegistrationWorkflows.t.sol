...
// contracts
import {Errors} from "lib/protocol-periphery-v1/contracts/libErrors.sol";
import {ISPGNFT} from "../../contracts/interfaces/ISPGNFT.sol";
import {SPGNFTLib} from "lib/protocol-periphery-v1/contracts/libSPGNFTLib.sol";
import {WorkflowStructs} from "lib/protocol-periphery-v1/contracts/libWorkflowStructs.sol";
...
contract RegistrationWorkflowsTest is BaseTest {
    ...
    function test_RegistrationWorkflows_createCollection()
        public
        withCollection
    {
    ...
    function test_RegistrationWorkflows_revert_mintAndRegisterIp_callerNotAuthorizedToMint()
        public
    {
    ...
    function test_RegistrationWorkflows_mintAndRegisterIp()
        public
        withCollection
        whenCallerHasMinterRole
    {
        ...
        (address ipId1, uint256 tokenId1) = registrationWorkflows
            .mintAndRegisterIp({
                spgNftContract: address(nftContract),
                recipient: u.bob,
                ipMetadata: ipMetadataEmpty
        ...
        assertEq(
            nftContract.tokenURI(tokenId1),
            string.concat(testBaseURI, tokenId1.toString())
        );
        assertMetadata(ipId1, ipMetadataEmpty);

        (address ipId2, uint256 tokenId2) = registrationWorkflows
            .mintAndRegisterIp({
                spgNftContract: address(nftContract),
                recipient: u.bob,
    ...
    function test_RegistrationWorkflows_registerIp() public {
        ...
        ) = _getSetPermissionSigForPeriphery({
                ipId: expectedIpId,
    ...
    function test_RegistrationWorkflows_multicall_mintAndRegisterIp()
        public
        withCollection
        whenCallerHasMinterRole
    {
        ...
        for (uint256 i = 0; i < 10; i++) {
            (ipIds[i], tokenIds[i]) = abi.decode(
                results[i],
                (address, uint256)
            );
            assertTrue(ipAssetRegistry.isRegistered(ipIds[i]));
            assertEq(
                nftContract.tokenURI(tokenIds[i]),
                string.concat(testBaseURI, ipMetadataDefault.nftMetadataURI)
            );
